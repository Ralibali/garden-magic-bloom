  ...
  try {
    ...
    for (const post of posts || []) {
      ...
      pages.push({
        ...
        type: 'article',
        image: absoluteUrl(post.cover_image_url || defaultImage),
        imageAlt: post.title,
    ...
    for (const plant of plants || []) {
      ...
      pages.push({
        ...
        type: 'article',
        image: absoluteUrl(plant.image_url || defaultImage),
    ...
    console.warn('[prerender] Skriver ändå first-byte för befintliga paket-URL:er så de inte kollapsar till startsidan.');
    return [];
  }
}
...
for (const page of allPages) await writePage(template, page);

const missingRequired = REQUIRED_FIRST_BYTE_PAGES.filter((required) => !allPages.some((page) => page.route === required.route));
if (missingRequired.length) {
  throw new Error(`[prerender] saknar obligatoriska first-byte-sidor: ${missingRequired.map((p) => p.route).join(', ')}`);
}

for (const required of REQUIRED_FIRST_BYTE_PAGES) {
  const written = allPages.find((page) => page.route === required.route);
  const html = await readFile(routeOutput(required.route), 'utf8');
  assertUniqueFirstByte(html, written);
}

console.log(`[prerender] skapade ${allPages.length} HTML-sidor (${staticPages.length} fasta, ${dynamicPages.length} dynamiska, ${REQUIRED_FIRST_BYTE_PAGES.length} first-byte-krav)`);
