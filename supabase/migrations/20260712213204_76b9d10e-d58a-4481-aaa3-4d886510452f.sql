...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.plant_care_events TO authenticated;
GRANT ALL ON public.plant_care_events TO service_role;
...
CREATE POLICY "Users can view own plant care events"
  ON public.plant_care_events FOR SELECT TO authenticated
...
CREATE POLICY "Users can create own plant care events"
  ON public.plant_care_events FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() = user_id
    AND EXISTS (
...
CREATE POLICY "Users can update own plant care events"
  ON public.plant_care_events FOR UPDATE TO authenticated
...
CREATE POLICY "Users can delete own plant care events"
  ON public.plant_care_events FOR DELETE TO authenticated
  USING (auth.uid() = user_id);
