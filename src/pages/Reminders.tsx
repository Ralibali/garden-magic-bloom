...
export default function Reminders() {
  ...
  const [newType, setNewType] = useState<ReminderType>('sowing');
  ...
  const { data: settingsData, isLoading } = useQuery({ queryKey: ['reminder-settings'], queryFn: getReminderBundle });
  const settings = ((settingsData?.settings as any) || {}) as { reminders?: GardenReminderRecord[]; notifications_enabled?: boolean; smart_action_state?: Record<string, any> };
  const reminders = settings.reminders || [];

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['reminder-settings'] });
  ...
  const requestNotifications = async () => {
    ...
    if (result === 'granted') {
      actionMutation.mutate(() => mergeReminderSettings({ notifications_enabled: true }));
      void recordProductActivity('reminder_notifications_enabled');
      toast({ title: 'Webbläsaraviseringar aktiverade', description: 'Du får en avisering om dagens och försenade uppgifter när du öppnar appen.' });
    }
  };

  const toggleDone = (reminder: GardenReminderRecord) => {
    const done = !reminder.done;
    actionMutation.mutate(() => updateReminder(reminder.id, { done, completed_at: done ? new Date().toISOString() : null }));
    void recordProductActivity(done ? 'reminder_completed' : 'reminder_reopened', { reminder_id: reminder.id });
  };

  const snooze = (reminder: GardenReminderRecord, days = 1) => {
    const today = localDateKey();
    const date = addDaysToDateKey(reminder.date < today ? today : reminder.date, days);
    actionMutation.mutate(() => updateReminder(reminder.id, { date }));
    void recordProductActivity('reminder_snoozed', { reminder_id: reminder.id, days });
    toast({ title: days === 1 ? 'Flyttad till imorgon' : `Flyttad ${days} dagar` });
  };

  const removeReminder = (id: string) => {
    actionMutation.mutate(() => deleteReminder(id));
    void recordProductActivity('reminder_deleted', { reminder_id: id });
  };

  const handleAdd = () => {
    if (!newTitle.trim() || !newDate) return;
    const reminder: GardenReminderRecord = { id: crypto.randomUUID(), title: newTitle.trim(), type: newType, date: newDate, done: false, created_at: new Date().toISOString(), completed_at: null };
  ...
  return <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
    <section className="premium-panel relative overflow-hidden p-5 sm:p-6">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><span className="section-kicker mb-3"><Sparkles className="h-3.5 w-3.5" /> Din personliga arbetslista</span><h1 className="page-title">Påminnelser</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Varje påminnelse sparas nu separat, så ändringar från flera enheter inte skriver över varandra.</p></div><Button onClick={() => setOpen(true)} className="gap-2"><Plus className="h-4 w-4" /> Ny påminnelse</Button></div>
      <div className="relative mt-6 grid grid-cols-3 gap-3 max-w-xl"><div className="metric-card p-3 sm:p-4"><p className="text-2xl font-bold">{upcoming.length}</p><p className="text-[9px] uppercase tracking-[0.1em] text-muted-foreground">kommande</p></div><div className="metric-card p-3 sm:p-4"><p className="text-2xl font-bold text-destructive">{urgent.length}</p><p className="text-[9px] uppercase tracking-[0.1em] text-muted-foreground">behöver fokus</p></div><div className="metric-card p-3 sm:p-4"><p className="text-2xl font-bold text-primary">{completedThisWeek}</p><p className="text-[9px] uppercase tracking-[0.1em] text-muted-foreground">klara 7 dagar</p></div></div>
    </section>

    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>Ny påminnelse</DialogTitle></DialogHeader><div className="space-y-4"><Input placeholder="Till exempel: Förodla tomater" value={newTitle} onChange={(event) => setNewTitle(event.target.value)} /><Select value={newType} onValueChange={(value) => setNewType(value as ReminderType)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="sowing">Sådd</SelectItem><SelectItem value="transplant">Utplantering</SelectItem><SelectItem value="watering">Vattning</SelectItem><SelectItem value="other">Övrigt</SelectItem></SelectContent></Select><Input type="date" value={newDate} onChange={(event) => setNewDate(event.target.value)} /><Button className="w-full" onClick={handleAdd} disabled={!newTitle.trim() || !newDate || actionMutation.isPending}>Spara påminnelse</Button></div></DialogContent></Dialog>
    ...
    {upcoming.length === 0 ? <AppEmptyState icon={Bell} eyebrow="Lugn arbetslista" title="Inga öppna påminnelser" description="Dagens smarta rekommendationer visas på översikten. Därifrån kan du skapa en påminnelse med ett tryck, eller lägga in något eget här." actionLabel="Skapa påminnelse" onAction={() => setOpen(true)} /> : <Card><CardHeader><CardTitle className="text-lg">Kommande</CardTitle></CardHeader><CardContent className="space-y-2">{upcoming.map((reminder) => { const config = typeConfig[reminder.type] || typeConfig.other; const Icon = config.icon; const days = daysUntil(reminder.date); return <div key={reminder.id} className="group flex items-center gap-3 rounded-2xl border border-border/60 bg-card/70 p-3 transition-colors hover:border-primary/20 sm:p-4"><button onClick={() => toggleDone(reminder)} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 border-border hover:border-primary" aria-label={`Markera ${reminder.title} som klar`} /><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${config.background}`}><Icon className={`h-4.5 w-4.5 ${config.color}`} /></div><div className="min-w-0 flex-1"><p className="font-semibold text-sm truncate">{reminder.title}</p><p className="mt-1 text-xs text-muted-foreground">{reminder.date} · {config.label}</p></div><Badge variant={days <= 0 ? 'destructive' : days <= 3 ? 'default' : 'secondary'} className="hidden sm:inline-flex">{days < 0 ? `${Math.abs(days)} d sen` : days === 0 ? 'Idag' : days === 1 ? 'Imorgon' : `${days} dagar`}</Badge><Button variant="ghost" size="sm" onClick={() => snooze(reminder)} disabled={actionMutation.isPending}><Clock3 className="h-3.5 w-3.5" /><span className="hidden lg:inline">Imorgon</span></Button><ConfirmDeleteButton itemName={reminder.title} description="Påminnelsen tas bort permanent." disabled={actionMutation.isPending} onConfirm={() => removeReminder(reminder.id)} /></div>; })}</CardContent></Card>}

    {completed.length > 0 && <Card><CardHeader><CardTitle className="text-lg text-muted-foreground">Avklarade</CardTitle></CardHeader><CardContent className="space-y-2">{completed.slice(0, 12).map((reminder) => { const config = typeConfig[reminder.type] || typeConfig.other; const Icon = config.icon; return <div key={reminder.id} className="flex items-center gap-3 rounded-2xl bg-muted/35 p-3 opacity-70"><button onClick={() => toggleDone(reminder)} className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-primary bg-primary/15"><Check className="h-3.5 w-3.5 text-primary" /></button><Icon className={`h-4 w-4 ${config.color}`} /><div className="min-w-0 flex-1"><p className="text-sm font-medium line-through truncate">{reminder.title}</p><p className="text-[10px] text-muted-foreground">Klar {reminder.completed_at ? new Date(reminder.completed_at).toLocaleDateString('sv-SE') : reminder.date}</p></div><ConfirmDeleteButton itemName={reminder.title} description="Påminnelsen tas bort permanent." disabled={actionMutation.isPending} onConfirm={() => removeReminder(reminder.id)} /></div>; })}</CardContent></Card>}
  </div>;
}
