...
export default function FieldJournalPage() {
  ...
  return <main id="main-content" className="field-journal min-h-screen bg-background pb-12">
    ...
    <Dialog open={!!editor} onOpenChange={open => { if (!open && !busy) void closeEditor(); }}><DialogContent className="max-w-lg max-h-[90dvh] overflow-y-auto"><DialogHeader><DialogTitle className="font-serif text-2xl">Ett ögonblick i odlingen</DialogTitle></DialogHeader>{editor && <form onSubmit={e => { e.preventDefault(); void save(); }} className="space-y-4">
