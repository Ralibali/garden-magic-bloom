...
export default function Sakalender() {
  ...
  const createCalendar = () => {
    const payload = { type: 'sakalender', zone, method, crops: selected, createdAt: new Date().toISOString() };
    try {
      localStorage.setItem('odlingsdagboken_public_sakalender', JSON.stringify(payload));
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {created && <section className="max-w-6xl mx-auto px-4 sm:px-8 py-14"><div className="mb-8"><p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold mb-3">Din plan</p><h2 className="font-serif text-3xl text-foreground mb-3">Här är din personliga såkalender</h2><p className="text-muted-foreground max-w-3xl leading-relaxed">Det här är en stark startpunkt för din odlingsplan. Justera alltid efter väder, frostnätter och hur dina plantor faktiskt utvecklas. {zoneText(zone)}</p></div><div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">{selectedCrops.map(crop => <article key={crop.name} className="bg-card border border-border rounded-2xl p-5 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="font-serif text-xl text-foreground">{crop.name}</h3><CalendarDays className="h-5 w-5 text-primary" /></div><dl className="space-y-3 text-sm"><div><dt className="font-medium text-foreground">Förodling</dt><dd className="text-muted-foreground">{crop.pre}</dd></div><div><dt className="font-medium text-foreground">Direktsådd/utplantering</dt><dd className="text-muted-foreground">{crop.plant}</dd></div><div><dt className="font-medium text-foreground">Skörd</dt><dd className="text-muted-foreground">{crop.harvest}</dd></div></dl><p className="mt-4 text-sm text-muted-foreground leading-relaxed border-t border-border pt-4">{crop.tip}</p></article>)}</div><div className="mt-10 grid lg:grid-cols-[1fr_360px] gap-4"><div className="bg-primary text-primary-foreground rounded-2xl p-6 sm:p-8"><h3 className="font-serif text-2xl mb-2">Spara din såkalender och få påminnelser</h3><p className="text-primary-foreground/85 mb-5 leading-relaxed">Din såkalender är sparad i webbläsaren. Skapa ett gratis konto på samma enhet så plockar Odlingsdagboken upp planen i appen.</p><Button asChild variant="secondary" size="lg" className="gap-2"><Link to="/login?mode=register&source=sakalender">Spara i gratis konto <ArrowRight className="h-4 w-4" /></Link></Button></div><div className="bg-card border border-border rounded-2xl p-6"><h3 className="font-serif text-xl text-foreground mb-2">Dela din såkalender</h3><p className="text-sm text-muted-foreground mb-4">Inspirera fler att komma igång med sin odling.</p><div className="rounded-xl bg-muted p-3 text-sm text-muted-foreground mb-4">{shareText}</div><Button variant="outline" className="w-full gap-2" onClick={async () => { await navigator.clipboard?.writeText(shareText); setCopied(true); }}><Copy className="h-4 w-4" /> {copied ? 'Kopierat!' : 'Kopiera text'}</Button></div></div></section>}
      </main>
    </div>
  );
}
