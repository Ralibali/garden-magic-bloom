...
export default function Sakalender() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border/50 bg-card/80 backdrop-blur-md sticky top-0 z-40"><div className="max-w-6xl mx-auto px-4 sm:px-8 h-14 flex items-center justify-between"><Link to="/" className="flex items-center gap-2 font-serif font-semibold text-foreground"><Sprout className="h-5 w-5 text-primary" /> Odlingsdagboken</Link><Button asChild size="sm"><Link to="/login?mode=register">Börja gratis</Link></Button></div></header>
      <main>
        <section className="bg-gradient-to-br from-background via-primary/5 to-accent/10"><div className="max-w-6xl mx-auto px-4 sm:px-8 py-14 sm:py-20 grid lg:grid-cols-[1fr_420px] gap-10 items-start"><div><p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold mb-4">Gratis odlingsverktyg</p><h1 className="font-serif text-4xl sm:text-5xl text-foreground leading-tight mb-5">Skapa din personliga såkalender för 2026</h1><p className="text-lg text-muted-foreground leading-relaxed max-w-2xl mb-6">Välj klimatzon, odlingssätt och vad du vill odla. Odlingsdagboken hjälper dig få koll på när det är dags att så, förodla och plantera ut – anpassat för svenska förhållanden.</p><div className="flex flex-wrap gap-3 text-sm text-muted-foreground"><span className="flex items-center gap-1.5"><Check className="h-4 w-4 text-primary" /> Svensk klimatzon</span><span className="flex items-center gap-1.5"><Check className="h-4 w-4 text-primary" /> Pallkrage, växthus och friland</span><span className="flex items-center gap-1.5"><Check className="h-4 w-4 text-primary" /> Sparas när du skapar konto</span></div></div><div className="bg-card border border-border rounded-2xl shadow-xl p-5 sm:p-6"><h2 className="font-serif text-2xl text-foreground mb-1">Vad vill du odla i år?</h2><p className="text-sm text-muted-foreground mb-5">Gör tre enkla val och få en tydlig startplan.</p><label className="text-sm font-medium text-foreground">Välj klimatzon</label><select value={zone} onChange={event => setZone(event.target.value)} className="mt-2 mb-4 w-full h-11 rounded-lg border border-input bg-background px-3 text-sm">{['1','2','3','4','5','6','7','8','Vet inte'].map(item => <option key={item} value={item}>{item === 'Vet inte' ? 'Jag vet inte' : `Zon ${item}`}</option>)}</select><label className="text-sm font-medium text-foreground">Hur odlar du?</label><div className="grid grid-cols-2 gap-2 mt-2 mb-4">{methods.map(item => <button key={item} type="button" onClick={() => setMethod(item)} className={`rounded-lg border px-3 py-2 text-sm transition-colors ${method === item ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:bg-muted'}`}>{item}</button>)}</div><label className="text-sm font-medium text-foreground">Välj växter</label><div className="grid grid-cols-2 gap-2 mt-2 mb-5 max-h-56 overflow-auto pr-1">{crops.map(crop => <button key={crop.name} type="button" onClick={() => setSelected(current => current.includes(crop.name) ? current.filter(item => item !== crop.name) : [...current, crop.name])} className={`rounded-lg border px-3 py-2 text-sm text-left transition-colors ${selected.includes(crop.name) ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:bg-muted'}`}>{crop.name}</button>)}</div><Button onClick={createCalendar} disabled={selected.length === 0} className="w-full gap-2" size="lg">Skapa min såkalender <ArrowRight className="h-4 w-4" /></Button></div></div></section>
        {created && <section className="max-w-6xl mx-auto px-4 sm:px-8 py-14"><div className="mb-8 rounded-2xl border border-primary/20 bg-primary/10 p-5"><p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold mb-2">Steg 1 av 2 klart</p><h2 className="font-serif text-3xl text-foreground mb-2">Din såkalender är klar 🌱</h2><p className="text-muted-foreground max-w-3xl leading-relaxed">Nu återstår bara att spara den, så kan du få påminnelser och följa upp vad som faktiskt fungerade. {zoneText(zone)}</p></div><div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">{selectedCrops.map(crop => <article key={crop.name} className="bg-card border border-border rounded-2xl p-5 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="font-serif text-xl text-foreground">{crop.name}</h3><CalendarDays className="h-5 w-5 text-primary" /></div><dl className="space-y-3 text-sm"><div><dt className="font-medium text-foreground">Förodling</dt><dd className="text-muted-foreground">{crop.pre}</dd></div><div><dt className="font-medium text-foreground">Direktsådd/utplantering</dt><dd className="text-muted-foreground">{crop.plant}</dd></div><div><dt className="font-medium text-foreground">Skörd</dt><dd className="text-muted-foreground">{crop.harvest}</dd></div></dl><p className="mt-4 text-sm text-muted-foreground leading-relaxed border-t border-border pt-4">{crop.tip}</p></article>)}</div><div className="mt-10 grid lg:grid-cols-[1fr_380px] gap-4"><div className="bg-primary text-primary-foreground rounded-2xl p-6 sm:p-8"><h3 className="font-serif text-2xl mb-2">Steg 2: spara planen och få påminnelser</h3><p className="text-primary-foreground/85 mb-5 leading-relaxed">Din såkalender är sparad i webbläsaren. Skapa ett gratis konto på samma enhet så plockar Odlingsdagboken upp planen i appen.</p><Button asChild variant="secondary" size="lg" className="gap-2"><Link to="/login?mode=register&source=sakalender">Spara planen och få påminnelser <ArrowRight className="h-4 w-4" /></Link></Button></div><PublicEmailCapture source="sakalender" plan={planPayload} title="Vill du få såkalendern skickad till dig?" description="Spara din e-post och fortsätt när det passar. När du skapar konto på samma enhet finns planen redo i appen." /></div><div className="mt-4 bg-card border border-border rounded-2xl p-6"><h3 className="font-serif text-xl text-foreground mb-2">Dela din såkalender</h3><p className="text-sm text-muted-foreground mb-4">Inspirera fler att komma igång med sin odling.</p><div className="rounded-xl bg-muted p-3 text-sm text-muted-foreground mb-4">{shareText}</div><Button variant="outline" className="gap-2" onClick={async () => { await navigator.clipboard?.writeText(shareText); setCopied(true); }}><Copy className="h-4 w-4" /> {copied ? 'Kopierat!' : 'Kopiera text'}</Button></div></section>}
      </main>
    </div>
  );
}
