...
import { approximateDataUrlBytes, compressImageFile, imageUrlToDataUrl } from '@/lib/images';
import { recordProductActivity } from '@/lib/analytics';
import { buildPhotoGroPrompt, photoGroModes, PhotoGroMode } from '@/lib/photoGro';
...
export default function PhotoDiary() {
  ...
  const [groMode, setGroMode] = useState<PhotoGroMode>('quick_review');
  ...
  const uploadMutation = useMutation({
    mutationFn: async (options?: { analyze?: boolean }) => {
      if (!selectedFile) throw new Error('Välj en bild först');
      const userId = await getUserId();
      const takenAt = localDateKey();
      const selectedBed = (beds as any[]).find((bed) => bed.id === bedId);
      let imageData: string | null = null;
      let prompt: string | null = null;

      if (options?.analyze) {
        imageData = await compressImageFile(selectedFile);
        if (approximateDataUrlBytes(imageData) > MAX_GRO_IMAGE_BYTES) throw new Error('Bilden blev för stor för Gro');
        prompt = buildPhotoGroPrompt({
          mode: groMode,
          takenAt,
          caption: caption.trim() || null,
          bedName: selectedBed?.name || null,
        });
      }
      ...
      return { id: data.id, analyze: !!options?.analyze, imageData, prompt };
    },
    onSuccess: (photo) => {
      ...
      setCaption('');
      setBedId('');
      void recordProductActivity('garden_photo_uploaded', { photo_id: photo.id, has_caption: !!caption.trim(), has_bed: !!bedId, analyze_after_upload: photo.analyze, gro_mode: groMode });
      if (photo.analyze && photo.imageData && photo.prompt) {
        void recordProductActivity('photo_first_gro_started', { photo_id: photo.id, gro_mode: groMode });
        toast({ title: 'Foto sparat — öppnar Gro 🌿' });
        navigate('/app/gro', { state: { prompt: photo.prompt, imageData: photo.imageData, source: 'photo_first_log', photoId: photo.id } });
        return;
      }
  ...
  const analyzeWithGro = async (photo: any) => {
    ...
    try {
      ...
      const prompt = buildPhotoGroPrompt({
        ...
        bedName: photo.beds?.name,
      });
      void recordProductActivity('garden_photo_opened_in_gro', { photo_id: photo.id, source: 'photo_diary_card' });
